  ...
  return (
    <div className="space-y-8">
      ...
      <div className="card-gray py-8 px-6">
        <h2 className="font-sans text-xl font-bold mb-6">next schedule:</h2>
        {booked.length === 0 ? (
          <p className="text-gray-500 font-sans text-base">No upcoming schedules</p>
        ) : (
          <div className="flex gap-6 overflow-x-auto pb-2">
            {booked.slice(0, 4).map((b: any, i: number) => (
              <div key={i} className="flex flex-col items-center min-w-[120px]">
                ...
                <p className="font-sans text-base italic">{b.DoctorName || 'doctor-name'}</p>
      ...
      <div className="space-y-4">
        {doctors.map((doc) => (
          <div key={doc.DoctorDetail.Id} className="card-yellow flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-sans italic text-base">{doc.DoctorDetail.FullName || 'doctor-name'}</p>
              <p className="font-sans text-base font-bold mt-1">doctor's details:</p>
              ...
              <p className="font-sans text-base font-bold">{doc.DoctorDetail.Fees ? 'active' : 'leave'}</p>
            </div>
        ...
        {doctors.length === 0 && (
          <div className="card-yellow text-center py-12">
            <p className="text-gray-500 font-sans text-lg">No doctors available</p>
          </div>
        )}
      </div>
      ...
      {/* Pagination */}
      {doctors.length > 0 && (
        <div className="flex justify-center gap-2 font-sans text-lg italic text-gray-500">
